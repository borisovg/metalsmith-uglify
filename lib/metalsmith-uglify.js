...
plugin = function (options) {
  ...
  // TODO add banner/footer options
  ...
  return function (files, metalsmith, done) {
    ...
    // It doesn't seem that files can ever be an non object...
    /* istanbul ignore if */
...
parse = function (options, files, filepaths, done) {
  ...
  if (options.concat && _.isString(options.concat)) {
...
parseAll = function (options, files, filepaths, done) {
  ...
  if (options.inSourceMap) {
    options.inFromString = true;
    // TODO check for errors, file existence, etc.
    options.inSourceMap = files[options.inSourceMap].contents.toString();
...
processSourceMap = function (options, uglified, files, dest) {
  ...
  var sourceMappingUrl = relativePath(dest)(options.outSourceMap)
    ...
    ;
  sourceMap.file = sourceMappingUrl;
